...
const MonthlySummaryView = ({ c, currency }) => (
    ...
    {typeof c.estimatedMonthlySavings === "number" && (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <Stat
          ...
          value={`${formatCurrency(Number(c.estimatedMonthlySavings || 0), currency)}/mo`}
          accent="emerald"
        />
        <Stat
...
const SavingsTipsView = ({ c, currency }) => {
      ...
      {totalSavings > 0 && (
        <div className="grid grid-cols-2 gap-3">
          <Stat
            ...
            value={`${formatCurrency(totalSavings, currency)}/mo`}
            accent="emerald"
          />
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {savings > 0 && (
                  <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full">
                    <TrendingUp size={11} />~{formatCurrency(savings, currency)}
                    /mo
                  </span>
                )}
              </div>
...
const headerChip = (insight, currency = "USD") => {
  const c = insight.content_json;
  if (
    ...
    <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden hover:border-slate-200 transition">
      ...
      {expanded && (
        <div className="px-5 pb-6 border-t border-slate-100 pt-5">
          {insight.insight_type === "monthly_summary" && (
            <MonthlySummaryView c={c} currency={currency} />
          )}
          {insight.insight_type === "budget_alert" && <BudgetAlertView c={c} />}
          {insight.insight_type === "savings_tips" && (
            <SavingsTipsView c={c} currency={currency} />
          )}
        </div>
      )}
    </div>
