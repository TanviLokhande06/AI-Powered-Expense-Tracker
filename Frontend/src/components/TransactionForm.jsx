...
import { getId, getTransactionDate } from "../utils/mongoCompat.js";
...
const TransactionForm = ({ initial, categories, onSaved, onCancel }) => {
  const [form, setForm] = useState({
    ...
    notes: initial?.notes || "",
    transactionDate:
      getTransactionDate(initial)?.split?.("T")?.[0] || todayDateString(),
  });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (initial) {
        await api.put(API_PATHS.TRANSACTIONS.UPDATE(getId(initial)), payload);
