...
const Insights = () => {
  ...
  const fetchInsights = async () => {
    try {
      ...
      // Normalize backend (Mongo) insight fields to the frontend shape
      const normalized = (res.data || []).map((i) => ({
        ...i,
        id: i.id || i._id || (i._id && String(i._id)),
        insight_type: i.insight_type || i.insightType || i.type || null,
        content_json: i.content_json || i.content || i.content_json || null,
        created_at: i.created_at || i.createdAt || i.created_at || null,
      }));
        ...
        <KpiCard
          ...
          value={
            stats.potentialSavings > 0
              ? `${formatCurrency(stats.potentialSavings, currency)}/mo`
              : "—"
          }
          icon={Wallet}
          accent="orange"
