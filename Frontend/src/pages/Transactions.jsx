...
const Transactions = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const normalizedTxns = (tRes.data || []).map((t) => ({
        ...t,
        id: t.id || t._id || (t._id && String(t._id)),
        _id: t._id || t.id || (t.id && String(t.id)),
        amount: t.amount || 0,
        transactionDate:
          t.transactionDate || t.transaction_date || t.date || null,
      ...
      setAllTransactions(normalizedTxns);
  ...
  const trendData = useMemo(() => {
    const now = new Date();
    const txnKey = (t) => (getTransactionDate(t) || "").split("T")[0];
  ...
  const generateInsight = async () => {
    ...
    try {
      const ids = transactions.slice(0, 50).map((t) => getId(t));
