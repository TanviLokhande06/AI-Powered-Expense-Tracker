...
import { getId } from "../utils/mongoCompat.js";
...
const Budgets = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const normalizedBudgets = (bRes.data || []).map((b) => ({
        ...b,
        id: b.id || b._id || (b._id && String(b._id)),
        _id: b._id || b.id || (b.id && String(b.id)),
        category_name:
          b.category_name ||
          (b.categoryId && b.categoryId.name) ||
          "Uncategorized",
        category_icon:
          b.category_icon || (b.categoryId && b.categoryId.icon) || null,
        category_color:
          b.category_color || (b.categoryId && b.categoryId.color) || null,
        spent: b.spent || 0,
        amount: b.amount || 0,
        period: b.period || "monthly",
      }));
      const normalizedCategories = (cRes.data || []).map((c) => ({
        ...c,
        id: c.id || c._id || (c._id && String(c._id)),
        _id: c._id || c.id || (c.id && String(c.id)),
        name: c.name,
        icon: c.icon,
        color: c.color,
        type: c.type,
  ...
  const analyzeAll = async () => {
    ...
    try {
      ...
      (res.data.analyses || []).forEach((a) => {
        const key = a.budgetId || a.budget_id || a.budget || a.id || a._id;
        map[String(key)] = a;
      });
      setAnalyses(map);
    } catch (err) {
            ...
            const barColor =
              pct >= 100
                ...
                : pct >= 70
                  ? "bg-amber-500"
                  : "bg-emerald-500";
            const analysis = analyses[getId(b)];
